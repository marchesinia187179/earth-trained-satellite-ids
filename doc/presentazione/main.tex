...
\begin{frame}{Preprocessing \& Datasets}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \resizebox{!}{4.5cm}{
        \begin{tikzpicture}
          \node[node_primary, minimum width=4cm] (sourceModel) {
            \textbf{Source-only}\\
            \\
          ...
          \node[node_accent, below=0.3cm of injModel, minimum width=4cm] (hybridModel) {
            \textbf{Hybrid}\\
            \\
            Ben: $100\%\mathcal{D}_S$\\
...
\begin{frame}{Generalizzazione vs Specializzazione}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \footnotesize DT: $-0.162$ \;\; HGB: $-0.094$ \;\; RF: $-0.172$
    \end{column}
    ...
    \begin{column}{0.5\textwidth}
      ...
      \footnotesize DT: $-0.079$ \;\; HGB: $-0.255$ \;\; RF: $-0.153$
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Risposte alle Domande Sperimentali}
  \tiny
    \begin{tabularx}{\linewidth}{>{\centering\arraybackslash}p{0.5cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
        \toprule
        \textbf{\color{unimoreMaroon}RQ} & \textbf{\color{unimoreMaroon}Risposta} & \textbf{\color{unimoreMaroon}Evidenza} & \textbf{\color{unimoreMaroon}Limite} \\
        \midrule
        \color{unimoreMaroon}\textbf{RQ1} &
        ...
        \addlinespace
        \addlinespace
        \color{unimoreMaroon}\textbf{RQ2} &
        Conoscenza target sparsa associata a una maggiore generalizzazione cross-domain. &
        La configurazione \textit{INJECTION} è associata a una maggiore sensibilità target, preservando sostanzialmente la risposta su NB15. &
        Il $\approx 0.3\%$ dei campioni non definisce una quantità minima od ottimale. \\
        \addlinespace
        \addlinespace
        \color{unimoreMaroon}\textbf{RQ3} &
...
% ------------------------------------------------------------------ %
% 10) TAKEAWAYS E FUTURO
% ------------------------------------------------------------------ %
\begin{frame}{Takeaways e Sviluppi Futuri}
  \begin{columns}[T]
    % TAKEAWAYS
    \begin{column}{0.5\textwidth}
      \centering
      \resizebox{0.7\textwidth}{!}{
        \begin{tikzpicture}[
          takeaway_box/.style={node_transform, minimum width=6cm, align=center, font=\scriptsize\bfseries},
          takeaway_label/.style={above, font=\scriptsize\bfseries, text=unimoreMaroon}
        ]

          % TAKEAWAY 1
          \node[takeaway_box] (t1) at (0,0) {La prestazione in-domain non\\ garantisce robustezza cross-domain};
          \node[takeaway_label] at (t1.north) {I Takeaway};

          % TAKEAWAY 2
          \node[takeaway_box, below=0.7cm of t1] (t2) {Una conoscenza target estremamente\\ limitata può favorire la generalizzazione\\ preservando il sorgente};
          \node[takeaway_label] at (t2.north) {II Takeaway};

          % TAKEAWAY 3
          \node[takeaway_box, below=0.7cm of t2] (t3) {La robustezza è un compromesso\\ tra adaptation e retention};
          \node[takeaway_label] at (t3.north) {III Takeaway};
        \end{tikzpicture}
      }
    \end{column}
    ...
    \begin{column}{0.5\textwidth}
      \resizebox{0.9\textwidth}{!}{
        \begin{tikzpicture}[
            future_dot/.style={circle, fill=unimoreMaroon, text=white, font=\bfseries\footnotesize, inner sep=2.2pt},
            future_label/.style={font=\footnotesize, align=left}
        ]

            % Coordinate dei tre punti
            \coordinate (p1) at (0,0);
            \coordinate (p2) at (2.4,1.1);
            \coordinate (p3) at (3.75,3);

            % Freccia curva
            \draw[arrow_flow] (0,0) to[bend right=35] (4,4);

            % Pallino 1
            \node[future_dot] at (p1) {1};
            \node[future_label, anchor=west] at ($(p1)+(0.2,-0.2)$) {
              \textbf{Estendere il domain shift}
            };

            % Pallino 2
            \node[future_dot] at (p2) {2};
            \node[future_label, anchor=west] at ($(p2)+(0.2,-0.2)$) {
              \textbf{Granularità Injection}\\ 
              \textbf{e trade-off decisionale}
            };

            % Pallino 3
            \node[future_dot] at (p3) {3};
            \node[future_label, anchor=west] at ($(p3)+(0.2,0)$) {
              \textbf{Valutare costi}\\
              \textbf{computazionali e}\\
              \textbf{deployment}
            };
        \end{tikzpicture}
      }
  ...
  \begin{center}
    \footnotesize \color{unimoreMaroon}\textbf{Adattarsi a nuove distribuzioni senza perdere la conoscenza acquisita.}
  \end{center}
\end{frame}
