...
La presente appendice raccoglie il dettaglio numerico delle principali analisi sperimentali sviluppate nel Capitolo~\ref{ch:risultati}.
Al fine di preservare la leggibilità della discussione principale, nel Capitolo~\ref{ch:risultati} sono riportati i risultati necessari all'interpretazione e al confronto tra i paradigmi sperimentali, mentre nel seguito sono raccolte le principali matrici, statistiche descrittive ed evidenze numeriche di supporto.
...
La Tabella~\ref{tab:baseline_f1_prauc_seed42} completa la caratterizzazione della baseline riportando i valori di \textit{F1-Score} e Average Precision (AP) ottenuti dai tre classificatori sui medesimi $16$ test set per il seed $s=42$.

\begin{table}[H]
    \centering
    \footnotesize
    \caption{F1-Score e Average Precision (AP) completi della baseline \texttt{INJECTION} con DT, HGB e RF sui $16$ test set per il seed $s=42$.}
    \label{tab:baseline_f1_prauc_seed42}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{lcccccc}
        \toprule
        & \multicolumn{2}{c}{\textbf{DT}} & \multicolumn{2}{c}{\textbf{HGB}} & \multicolumn{2}{c}{\textbf{RF}} \\
        \textbf{Test Set} & \textbf{F1-Score} & \textbf{Average Precision (AP)} & \textbf{F1-Score} & \textbf{Average Precision (AP)} & \textbf{F1-Score} & \textbf{Average Precision (AP)} \\
        \midrule
...
\begin{table}[H]
    \centering
    \footnotesize
    \caption{Media e varianza della TNR sui $10$ seed per i modelli sorgente. Nel protocollo adottato, il valore è indipendente dal test set malevolo poiché la componente nominale rimane invariata e deriva da \texttt{NB15}.}
...
Il migliore modello sorgente viene selezionato a posteriori per ciascun benchmark e famiglia algoritmica sulla base dei risultati osservati; il confronto è pertanto descrittivo e non rappresenta una procedura di selezione disponibile in deployment senza accesso alle etichette target.

\begin{table}[H]
    \centering
    \footnotesize
    \caption{Differenza di TPR tra la baseline e il migliore modello sorgente disponibile su ciascun benchmark target.}
...
\begin{table}[H]
    ...
    \begin{tabular}{lccccccccc}
        ...
        \textbf{Modello} & \textbf{NB15} & \textbf{INJECTION} & \textbf{Target} & \textbf{NB15} & \textbf{INJECTION} & \textbf{Target} & \textbf{NB15} & \textbf{INJECTION} & \textbf{Target} \\
        \midrule
...
\section{Esiti dei Test Statistici}
\label{app:welch}

La presente sezione raccoglie le statistiche descrittive e gli esiti registrati dalla procedura dei test $t$ di Welch discussi nel Capitolo~\ref{ch:risultati}. 
...
La quantità globale confrontata corrisponde, per ciascun seed, alla media a peso uniforme dei valori F1 ottenuti sui $16$ benchmark della cross-evaluation; non rappresenta un F1 pooled sulle singole predizioni né una stima pesata secondo prevalenze operative reali.
...
I valori di F1 utilizzati dalla procedura derivano dalle metriche persistite dalla pipeline con l'arrotondamento configurato mediante \texttt{MLConstants.DECIMAL\_DIGITS}. La procedura \texttt{ttest\_ind(..., equal\_var=False)} tratta i vettori confrontati come campioni indipendenti.

\begin{table}[H]
    \centering
    \footnotesize
    \caption{Media e deviazione standard campionaria dei valori $\overline{F1}_{i}^{(k)}$ sui $10$ seed per la baseline e i modelli sorgente. $\Delta_{\mathrm{base}}\overline{F1}$ è calcolato come baseline meno modello sorgente.}
    \label{tab:sorgente_welch_descriptive}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{lccccccccc}
        \toprule
         & \multicolumn{3}{c}{\textbf{DT}} & \multicolumn{3}{c}{\textbf{HGB}} & \multicolumn{3}{c}{\textbf{RF}} \\
        \textbf{Modello} & $\boldsymbol{\overline{\overline{F1}}}$ & $\boldsymbol{\mathrm{SD}(F1)}$ & $\boldsymbol{\Delta_{\mathrm{base}}\overline{F1}}$ & $\boldsymbol{\overline{\overline{F1}}}$ & $\boldsymbol{\mathrm{SD}(F1)}$ & $\boldsymbol{\Delta_{\mathrm{base}}\overline{F1}}$ & $\boldsymbol{\overline{\overline{F1}}}$ & $\boldsymbol{\mathrm{SD}(F1)}$ & $\boldsymbol{\Delta_{\mathrm{base}}\overline{F1}}$ \\
        \midrule
...
I $p$-value riportati sono nominali e non sottoposti a correzione per confronti multipli; la colonna ``Sign.'' applica la soglia $\alpha=0{,}05$ in chiave esplorativa.
...
\begin{table}[H]
    ...
    \begin{tabular}{lccccccccc}
        \toprule
         & \multicolumn{3}{c}{\textbf{DT}} & \multicolumn{3}{c}{\textbf{HGB}} & \multicolumn{3}{c}{\textbf{RF}} \\
        \textbf{Modello} & $\boldsymbol{t}$ & $\boldsymbol{p}$ & \textbf{Sign.} & $\boldsymbol{t}$ & $\boldsymbol{p}$ & \textbf{Sign.} & $\boldsymbol{t}$ & $\boldsymbol{p}$ & \textbf{Sign.} \\
        \midrule
...
\begin{table}[H]
    \centering
    \footnotesize
    \caption{Media e deviazione standard campionaria dei valori medi di F1-Score sui $10$ seed per la baseline e i modelli ibridi. $\Delta_{\mathrm{base}}\overline{F1}$ è calcolato come baseline meno modello ibrido.}
...
Anche in questo caso, i $p$-value sono nominali, senza correzione per confronti multipli, e la colonna ``Sign.'' applica il criterio $\alpha=0{,}05$ con finalità esplorativa.
