...
Con l'affermazione di un paradigma trasmissivo interamente digitale, la convergenza di supporti fisici elettrici, ottici ed elettromagnetici su topologie geografiche complesse ha determinato un progressivo ampliamento della superficie d'attacco\footnote{La superficie d'attacco definisce l'insieme complessivo dei vettori di ingresso, delle vulnerabilità software, delle interfacce di rete e dei nodi fisici attraverso cui un agente malevolo può tentare di accedere, degradare o alterare un sistema di comunicazione.}.
...
Da un lato, la combinazione di mezzi guidati ad elevata ampiezza di banda — quali le fibre ottiche — e di collegamenti radio consente di realizzare infrastrutture caratterizzate da elevata capacità trasmissiva e, in molti scenari, da latenze contenute, rendendo tali reti particolarmente adatte alle applicazioni in tempo reale \cite{tanenbaum2020}.
...
Sebbene i protocolli crittografici a livello di rete e trasporto (quali IPsec e TLS\footnote{IPsec (\textit{Internet Protocol Security}) opera a livello di rete, proteggendo il traffico IP mediante modalità che possono interessare il solo payload o l\'intero pacchetto IP; TLS (\textit{Transport Layer Security}) opera invece sopra il livello di trasporto, tipicamente su TCP, fornendo proprietà crittografiche al canale applicativo.}) garantiscano la riservatezza e l'integrità dei dati contro l'ascolto passivo, essi si dimostrano intrinsecamente insufficienti a proteggere l'infrastruttura nella sua interezza. 
...
Risulta quindi rilevante integrare sistemi di monitoraggio attivo, come i \textit{Network Intrusion Detection Systems} (NIDS), capaci di analizzare continuamente i flussi di rete per identificare tempestivamente deviazioni dal comportamento nominale prima che si traducano in disservizi sistemici \cite{khraisat2019}.
...
In tale contesto di integrazione, l'impiego di piattaforme orbitali rappresenta una soluzione di particolare rilevanza architetturale su vasta scala per garantire una copertura geografica globale, introducendo tuttavia severe sfide operative e di sicurezza. 
...
Se da un lato i collegamenti in orbita geostazionaria (GEO) sono penalizzati da un elevato ritardo di propagazione che degrada le prestazioni dei meccanismi di controllo della congestione del TCP, dall'altro le costellazioni in orbita bassa (LEO) possono introdurre rilevanti scostamenti Doppler nelle bande operative e frequenti procedure di \textit{handoff}, conseguenti al limitato intervallo di visibilità dei singoli satelliti \cite{kodheli2021}.
...
Per rispondere a questa crescente complessità, l'integrazione di architetture \textit{Network Intrusion Detection Systems} (NIDS) rappresenta una possibile strategia per l'ispezione continua e non invasiva del traffico trasversale alle tratte terrestri e orbitali. 
Tuttavia, la massiva volumetria dei flussi e le fluttuazioni stocastiche del canale radio possono accentuare i limiti dei tradizionali NIDS basati su firme (\textit{signature-based}\footnote{I sistemi NIDS basati su firme, quali ad esempio Snort o Suricata, eseguono un'ispezione deterministica del traffico confrontando il contenuto dei pacchetti con un database di regole e pattern malevoli predefiniti, presentando limitazioni nel riconoscimento di variazioni non contemplate dalle regole, minacce non ancora catalogate e contenuti applicativi non direttamente ispezionabili in presenza di cifratura.}) nel riconoscimento di attacchi inediti (\textit{zero-day}) o di alterazioni comportamentali sottili.

In tale scenario, l\'adozione di modelli di Machine Learning rappresenta una possibile strategia per profilare statisticamente la dinamica del traffico e identificare deviazioni riconducibili ad attività malevole \cite{khraisat2019}.
L'apprendimento automatico può supportare l'analisi predittiva e la correlazione di sicurezza multi-dominio, affiancando i meccanismi di monitoraggio sia nelle tratte di superficie sia in quelle satellitari. 
...
Tuttavia, l'analisi della letteratura considerata evidenzia un limite metodologico ricorrente: numerosi studi valutano tali modelli prevalentemente all'interno del medesimo dominio utilizzato durante l\'addestramento, fornendo una caratterizzazione limitata del comportamento in condizioni cross-domain.

Un primo filone di ricerca nell'ambito del monitoraggio non presidiato poggia sull'analisi fondamentale di Sommer e Paxson \cite{sommer2010}, incentrata sull'analisi critica dell'impiego del ML nell'anomaly detection in reti di grande scala. 
...
Nel presente lavoro viene privilegiato l'apprendimento supervisionato, così da disporre di funzioni decisionali addestrate su osservazioni etichettate e di prestazioni quantitativamente valutabili anche in presenza di disallineamento distributivo.

Spostando l'attenzione sui modelli guidati da dati etichettati, un primo riferimento consolidato per il \textit{benchmarking} terrestre è rappresentato dallo studio di Tavallaee et al. \cite{tavallaee2009}. 
...
Nonostante l'importanza storica di tale lavoro nel validare l'accuratezza dei modelli supervisionati, il presente lavoro se ne discosta per il diverso obiettivo sperimentale: la valutazione condotta da Tavallaee et al. rimane circoscritta a un dominio sintetico e stazionario, senza indagare il comportamento dei classificatori di fronte a fenomeni di \textit{domain shift}\footnote{Con \textit{domain shift} (o scostamento distributivo) si intende una variazione della distribuzione di probabilità congiunta dei dati tra il dominio di addestramento e quello di test ($P_S(X,Y) \neq P_T(X,Y)$), condizione che può essere associata a un degrado della capacità di generalizzazione del modello.}.
...
In ambienti caratterizzati da tale variabilità, i classificatori supervisionati costituiscono una delle strategie utilizzabili per mantenere il processo decisionale ancorato a osservazioni etichettate e caratterizzarne quantitativamente il tasso di falsi allarmi (\textit{False Positive Rate}, FPR), fermo restando che la prestazione dipende dalla rappresentatività dei dati di addestramento \cite{khraisat2019, kodheli2021}. 
...
Il framework considerato nel presente elaborato non introduce invece procedure federate né meccanismi di sincronizzazione distribuita: tra i regimi analizzati viene valutata la trasferibilità diretta (\textit{zero-shot}\footnote{Nel contesto considerato, l'approccio \textit{zero-shot} indica la valutazione diretta delle prestazioni di un classificatore addestrato esclusivamente sul dominio sorgente nei confronti di distribuzioni target non osservate durante l'addestramento, senza applicare procedure di adattamento o riaddestramento supervisionato sul target.}) di classificatori supervisionati addestrati sul dominio terrestre e, separatamente, l'effetto dell'introduzione controllata di conoscenza target. 
...
\centering
\small
\caption{Sintesi comparativa dei principali approcci considerati nella letteratura per il rilevamento delle intrusioni e la gestione adattiva delle reti terrestri e integrate terrestre-spaziali, con posizionamento del presente lavoro.}
\label{tab:stato_arte_nids}
\begin{tabularx}{\textwidth}{l >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
\toprule
\textbf{Rif.} & \textbf{Dominio} & \textbf{Approccio} \\
\midrule
\cite{sommer2010} & Terrestre & Anomaly Detection / ML \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Contributo} Rilevamento di minacce \textit{zero-day} senza dataset etichettati.} \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Limite} Elevato tasso di falsi allarmi (FPR) dovuto alle fluttuazioni stocastiche di rete.} \\
\addlinespace
\cite{tavallaee2009} & Terrestre & ML Supervised \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Contributo} Benchmarking rigoroso e bonifica dei bias di ridondanza (NSL-KDD).} \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Limite} Analisi circoscritta a dati sintetici e stazionari; assenza di valutazione su \textit{domain shift}.} \\
\addlinespace
\cite{sharafaldin2018} & Terrestre & ML Supervised \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Contributo} Caratterizzazione di flussi ad alta dimensionalità (CIC-IDS2017).} \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Limite} Valutazione prevalentemente circoscritta al dominio di riferimento; trasferibilità cross-domain non analizzata.} \\
\addlinespace
\cite{wan2025} & Spazio-Terra & Federated Learning \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Contributo} Addestramento distribuito e cooperativo con tutela della privacy.} \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Limite} Richiede un'infrastruttura federata e procedure di sincronizzazione distribuita, differenti dal protocollo centralizzato considerato.} \\
\addlinespace
\cite{alam2024} & SAGIN & GNN + DRL \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Contributo} Adattamento dinamico alle topologie spaziali ad alta variabilità.} \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Limite} Obiettivo primario di orchestrazione delle risorse, distinto dalla valutazione cross-domain dei NIDS.} \\
\midrule
\textbf{Presente lavoro} & Terrestre / Terrestre-Spaziale & ML Supervised; Cross-Domain \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Contributo} Valutazione \textit{zero-shot}, conoscenza target sparsa, cross-evaluation e analisi \textit{multi-seed}.} \\
\multicolumn{3}{>{\raggedright\arraybackslash}p{\dimexpr\textwidth-2\tabcolsep\relax}}{\textbf{Limite} \textit{Benign-domain shift} non valutato; assenza di misure di latenza, memoria e consumo energetico per il deployment.} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
    ...
    \node [node_process] at (-2.35, 0) (a_train) {
        \textbf{Addestramento}\\[-5pt]
        \textbf{Classificatore ML}
    };
    \node [node_primary] at (2.35, 0) (a_test) {
        \textbf{Test Terrestre}
    };
    \node [node_database, left=1.5cm of a_train] (a_data) {
        \textbf{Dataset}\\[-5pt]
        \textbf{Terrestre}
    };
    \node [node_process, right=1.5cm of a_test] (a_res) {
        \textbf{Elevata}\\[-5pt]
        \textbf{Accuratezza}
    };
    ...
    \node [node_database, below=2cm of a_data] (b_data) {
        \textbf{Dataset}\\[-5pt]
        \textbf{Terrestre}
    };
    \node [node_process, right=1.5cm of b_data] (b_train) {
        \textbf{Addestramento}\\[-5pt]
        \textbf{Classificatore ML}
    };
    \node [node_accent, right=1.8cm of b_train] (b_test) {
        \textbf{Test Target}
    };
    \node [node_alert, right=0.85cm of b_test] (b_res) {
...
Assumendo uno spazio delle \textit{feature} condiviso ($\mathcal{X}_S=\mathcal{X}_T=\mathcal{X}$), il termine \textit{domain shift} indica in senso generale un disallineamento tra le distribuzioni associate ai domini sorgente e target. Una possibile manifestazione è la variazione delle distribuzioni marginali delle osservazioni, $P_S(X)\neq P_T(X)$; tale condizione, tuttavia, non è da sola sufficiente a identificare lo specifico caso di \textit{covariate shift}.
Un caso particolare è il \textit{covariate shift}, caratterizzato da
\[ P_S(X)\neq P_T(X), \qquad P_S(Y\mid X)=P_T(Y\mid X), \]
nel quale varia la distribuzione marginale delle osservazioni mantenendo invariata la relazione condizionata tra caratteristiche ed etichetta. 
Una variazione di quest'ultima relazione è invece descritta da
$P_S(Y\mid X)\neq P_T(Y\mid X)$
e identifica una forma di variazione condizionata del concetto, \textit{concept shift} \cite{pan2010}.

Nelle telecomunicazioni, tale discrepanza matematica può riflettersi in differenti profili statistici del traffico. 
Il passaggio tra contesti terrestri e satellitari può essere associato a differenze nei tempi di propagazione, nella disponibilità di banda e nei fenomeni di attenuazione, la cui entità dipende dalla specifica configurazione di rete \cite{kodheli2021, azar2023}. 
Tali proprietà operative possono riflettersi nella distribuzione dei tempi di interarrivo e nella struttura dei flussi, introducendo variazioni distributive nelle \textit{feature} estratte dal sensore NIDS. 
Geometricamente, tali differenze possono condurre le osservazioni del dominio target verso regioni dello spazio delle caratteristiche non adeguatamente rappresentate durante l\'addestramento sul dominio sorgente, modificando la risposta della funzione decisionale e risultando associate a un possibile degrado delle prestazioni di classificazione \cite{farahani2021}.
...
Tuttavia, tali approcci affrontano un problema differente rispetto a quello considerato nel presente lavoro, che è invece focalizzato sulla valutazione sperimentale della trasferibilità cross-domain di classificatori NIDS tra benchmark sorgente e target caratterizzati da differenti regimi di conoscenza target.
Di conseguenza, il presente lavoro affronta uno specifico problema emerso nella letteratura considerata: la valutazione della trasferibilità cross-domain di classificatori NIDS in presenza di marcate differenze tra le distribuzioni sorgente e target e di una disponibilità limitata di conoscenza target etichettata.

\subsection[Adattamento di dominio supervisionato in presenza di conoscenza target limitata]{Adattamento di dominio supervisionato in presenza di\\conoscenza target limitata}
...
Tale vincolo può configurare differenti regimi di apprendimento: scenari supervisionati a pochi campioni (\textit{few-shot}) quando è disponibile una quantità estremamente ridotta di osservazioni target etichettate, oppure scenari di adattamento non supervisionato quando le etichette target risultano completamente assenti.
...
Una possibile formulazione teorica dell'adattamento supervisionato consiste nel considerare contributi distinti del dominio sorgente e del dominio target all'interno di una funzione obiettivo comune \cite{daume2009, motiian2017}.
...
Una possibile formulazione astratta del principio di adattamento supervisionato considera una funzione obiettivo $\mathcal{L}(\theta)$ composta da contributi distinti associati ai domini sorgente e target:
\begin{equation}
\mathcal{L}(\theta) =
\frac{1}{N_S}
\sum_{i=1}^{N_S}
\ell\left(f(x_i^S;\theta),y_i^S\right)
+
\alpha
\frac{1}{N_T}
\sum_{j=1}^{N_T}
\ell\left(f(x_j^T;\theta),y_j^T\right),
\label{eq:supervised_da_loss}
\end{equation}
dove $\ell(\cdot)$ rappresenta una generica funzione di perdita istantanea, ad esempio la \textit{cross-entropy}, mentre $\alpha>0$ costituisce un iperparametro che controlla il peso relativo attribuito al contributo del dominio target rispetto a quello del dominio sorgente\footnote{L'iperparametro $\alpha$ modula il peso relativo attribuito al rischio empirico del dominio target rispetto a quello del dominio sorgente; esso non rappresenta necessariamente un parametro della pipeline sperimentale adottata nel presente lavoro.}.

In termini generali, la formulazione dell'Equazione~(\ref{eq:supervised_da_loss}) esprime il principio secondo cui l'informazione supervisionata disponibile nel dominio target può essere incorporata nel processo di apprendimento congiuntamente a quella proveniente dal dominio sorgente \cite{motiian2017}.
Essa costituisce pertanto un riferimento teorico per descrivere strategie di adattamento in presenza di una disponibilità limitata di campioni target etichettati.

L'efficacia di tale adattamento non può tuttavia essere assunta a priori, poiché dipende dalla relazione tra le distribuzioni coinvolte, dalla quantità e dalla rappresentatività della conoscenza target e dal classificatore adottato.
Nel presente lavoro, tale principio viene investigato sperimentalmente attraverso configurazioni controllate del \textit{training set}, descritte formalmente nel capitolo successivo, senza identificare l'Equazione~(\ref{eq:supervised_da_loss}) con una specifica funzione di perdita implementata nella pipeline.
