...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{
        ...
        Dominio di traffico e relativa distribuzione marginale delle osservazioni \\

        $\mathcal{T} = (\mathcal{Y}, P(Y \mid \mathbf{x}))$ &
        Task di classificazione e distribuzione condizionata delle etichette \\
...
Il framework analitico richiede la definizione rigorosa degli spazi di rappresentazione vettoriale, delle distribuzioni di probabilità associate ai diversi contesti di rete e del task supervisionato oggetto della valutazione. Si definisce un dominio $\mathcal{D}$ come la coppia
\begin{equation}
    \mathcal{D} = \left(\mathcal{X}, P(\mathbf{x})\right),
\end{equation}
dove $\mathcal{X} \subseteq \mathbb{R}^d$ rappresenta lo spazio delle \textit{feature} $d$-dimensionale e $P(\mathbf{x})$ la distribuzione marginale definita sul vettore delle caratteristiche $\mathbf{x} = (x_1, x_2, \dots, x_d)^\top \in \mathcal{X}$.

Al dominio è associato il task supervisionato
\begin{equation}
    \mathcal{T} = \left(\mathcal{Y}, P(Y \mid \mathbf{x})\right),
\end{equation}
dove lo spazio discreto $\mathcal{Y} = \{0,1\}$ stabilisce la natura del flusso: $Y = 0$ identifica il traffico nominale (\textit{Normal}), mentre $Y = 1$ identifica il traffico malevolo. In aggiunta, si introduce una variabile descrittiva $A \in \mathcal{A}$ che specifica la classe del flusso e, per $Y=1$, consente di distinguere le differenti famiglie di attacco.

Nel seguito, il termine \textit{domain shift} viene utilizzato in senso generale per indicare un disallineamento distributivo tra il contesto sorgente e quello target. 
Il \textit{covariate shift} ne costituisce un caso particolare, caratterizzato dalla variazione della distribuzione marginale delle osservazioni,
\begin{equation}
    P_S(\mathbf{x}) \neq P_T(\mathbf{x}),
\end{equation}
a fronte dell'invarianza della distribuzione condizionata delle etichette,
\begin{equation}
    P_S(Y \mid \mathbf{x}) = P_T(Y \mid \mathbf{x}).
\end{equation}
Il protocollo sperimentale adottato non assume né verifica quest'ultima uguaglianza. Inoltre, i domini considerati differiscono anche per provenienza e composizione delle famiglie di attacco. Di conseguenza, le differenze osservate vengono interpretate nel seguito come evidenze di \textit{domain shift} o disallineamento distributivo, senza ricondurle necessariamente al solo caso particolare di \textit{covariate shift}.
...
Per ridurre il rischio di contaminazione informativa tra la fase di addestramento e quella di valutazione (\textit{data leakage}\footnote{L'esecuzione dello split a monte di qualsiasi fusione previene il riutilizzo diretto delle medesime righe tra le partizioni di addestramento e test dei benchmark derivati.}), la partizione di ciascun dominio primario $\mathcal{D}_k$ nei sotto-insiemi di addestramento ($\mathcal{D}_k^{(train)}$) e di verifica ($\mathcal{D}_k^{(test)}$) viene definita a monte di qualsiasi aggregazione o composizione dei benchmark derivati. In tutte le configurazioni sperimentali viene adottata una suddivisione stocastica stratificata con proporzione fissa:
\begin{equation}
...
\end{equation}
La separazione mantiene disgiunti i record assegnati alle partizioni di training e test, prevenendo il riutilizzo diretto delle medesime osservazioni. L'eventuale dipendenza residua dovuta a flussi correlati, duplicati o acquisizioni comuni dipende tuttavia dalla struttura dei dataset originari e non è esclusa dalla sola disgiunzione delle righe.
...
\`E opportuno precisare che tale omissione riguarda \textbf{esclusivamente lo stadio di addestramento e inferenza dei classificatori}. Qualora strumenti diagnostici non invarianti alla scala (es. \textit{Principal Component Analysis} o stime di densità kernel, \textit{KDE}) vengano impiegati a valle per l'ispezione visiva del disallineamento distributivo tra domini o per finalità di spiegabilità, essi richiedono una normalizzazione dedicata, applicata \textbf{unicamente a scopo diagnostico-visivo} e non reintrodotta nello stadio di classificazione.
...
in cui la classe nominale $Y=0$ di origine è unita alla totalità dei vettori d'attacco $Y=1$ del contesto di destinazione ($\mathcal{D}_{T}^{(\text{train})}$). Il rispettivo vettore di parametri ottimali risponde alla formulazione:
    ...
    \end{equation}
    Tali modelli rappresentano una configurazione di riferimento a elevata esposizione alla componente malevola target disponibile. Essi non costituiscono un upper bound teorico né un Oracle sul dominio target completo, poiché il benchmark non include una componente benigna target e la famiglia di modelli esplorata è limitata alle configurazioni considerate nel protocollo.
\end{enumerate}
...
\end{equation}
Tale proprietà rende generalmente non necessaria, per i classificatori ad albero considerati, l'applicazione di trasformazioni di scala quali standardizzazione z-score o normalizzazione Min-Max ai fini della costruzione delle soglie decisionali.
...
\begin{equation}
    ...
    \label{eq:row_mean}
\end{equation}
La media di riga $\overline{H}_k(M)$ costituisce una macro-media non pesata sui benchmark del panel sperimentale: ciascun benchmark contribuisce con lo stesso peso, indipendentemente dalla propria numerosità campionaria e dalla natura sorgente o target. Essa deve pertanto essere interpretata come misura sintetica del comportamento del modello sull'insieme degli scenari considerati e non come stima della prestazione attesa sotto una specifica distribuzione operativa di traffico.
...
\begin{table}[H]
    \centering
    \small
    \caption{Rappresentazione concettuale della matrice di valutazione incrociata per una generica metrica $q$.}
...
Per ciascun modello $M$ (con esclusione del baseline stesso), sia $\overline{\text{F1}}_k(M) = \frac{1}{|\mathcal{B}|}\sum_{D\in\mathcal{B}} \text{F1}_k(M,D)$ la macro-media non pesata dell'F1-Score sui benchmark per il seed $s_k$, definita in analogia all'Equazione~\eqref{eq:row_mean} a partire dalla metrica dell'Equazione~\eqref{eq:precision_f1_def}. Siano $\{\overline{\text{F1}}_k(M)\}_{k=1}^{K}$ e $\{\overline{\text{F1}}_k(\mathcal{M}_{\text{base}})\}_{k=1}^{K}$ le rispettive sequenze di medie per seed per il modello $M$ e per il baseline. Definita l'ipotesi nulla $H_0: \mu_M = \mu_{\text{base}}$, la statistica del test è espressa come:
\begin{equation}
...
\subsubsection{Dinamica della Soglia e Analisi Grafica (ROC e Precision--Recall)}
...
A tale scopo si impiegano le curve \textit{Receiver Operating Characteristic} (ROC) e \textit{Precision--Recall} (PR). La curva ROC viene sintetizzata mediante la relativa area sottesa:
\begin{equation}
    AUC_{ROC}=\int_{0}^{1}TPR\,d(FPR).
\end{equation}

Per la curva Precision--Recall, l'implementazione adotta invece l'\textit{Average Precision} (AP), calcolata come:
\begin{equation}
    \mathrm{AP}=\sum_n (R_n-R_{n-1})P_n,
    \label{eq:average_precision}
\end{equation}
dove $P_n$ e $R_n$ rappresentano rispettivamente Precision e Recall al punto operativo $n$. Tale quantità non coincide con l'area ottenuta mediante interpolazione trapezoidale della curva Precision--Recall.
...
Il trasferimento di un modello tra domini caratterizzati da distribuzioni differenti può determinare una degradazione delle prestazioni di classificazione \cite{pan2010}. Nel presente protocollo tale disallineamento viene considerato nel senso generale di \textit{domain shift}, senza assumere che ricorrano necessariamente le condizioni più restrittive del \textit{covariate shift}, per il quale sarebbe inoltre richiesta l'invarianza $P_S(Y \mid \mathbf{x}) = P_T(Y \mid \mathbf{x})$. La variazione dell'efficacia discriminativa di un modello $M$, valutato sul dominio sorgente $\mathcal{D}_S$ e sul dominio target $\mathcal{D}_T$, può essere espressa come
\begin{equation}
