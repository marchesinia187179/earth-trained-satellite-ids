...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{
        ...
        $\mathcal{L}$ &
        Funzione BCE della formulazione astratta dell'apprendimento \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{
        ...
        $H_k$ &
        Matrice delle prestazioni ottenute con il seed $s_k$ \\
...
\begin{figure}[H]
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \node[below, font=\footnotesize] at (centro_asse) {Lo shift $\boldsymbol{\eta}$ viene assorbito dal confine.};
    \end{scope}
    ...
    \begin{scope}[shift={(8,0)}]
        ...
        % Didascalia concettuale
        \node[below, font=\footnotesize] at (centro_asse) {Lo shift $\boldsymbol{\eta}$ elude il confine decisionale.};
    \end{scope}
    \end{tikzpicture}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{
        ...
        Matrice di covarianza e proiezione PCA \\
        ...
        $I_j^{(D)}, C_j$ &
        Importanza SHAP media e frequenza nella \textit{top-3} \\

        $TP(\tau), FP(\tau), TN(\tau), FN(\tau)$ &
        Conteggi della matrice di confusione alla soglia $\tau$ \\

        $AUC_{ROC}, AUC_{PR}$ &
        Aree sottese alle curve ROC e Precision--Recall \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{4cm} >{\raggedright\arraybackslash}X }
        ...
        Riduzione della dominanza delle scale numeriche: la matrice di covarianza $\boldsymbol{\Sigma}_D$ richiede unità adimensionali; in caso contrario, le componenti principali catturano solo la scala numerica grezza più ampia. \\
        \addlinespace
        \textbf{KDE} &
...
Attraverso l'allineamento degli spazi delle \textit{feature}, la formalizzazione dei modelli di classificazione (aggregati e specialistici) e la definizione di un protocollo stocastico fondato su valutazioni \textit{multi-seed} e test di significatività, il framework garantisce di mantenere distinta la variabilità dal rumore di misura.
