...
L'infrastruttura tecnica descritta nel presente capitolo costituisce il supporto computazionale ed esecutivo dell'intero framework metodologico. 
Al fine di garantire la totale trasparenza e la riproducibilità end-to-end degli esperimenti, la presente sezione dettagli la struttura della repository software, le dipendenze tecnologiche impiegate e i meccanismi di controllo dell'aleatorietà stocastica.
...
\hline
\texttt{Python}\footnote{\url{https://www.python.org/}} & \texttt{3.14.4} & Ambiente di esecuzione e interprete base \\
\texttt{pandas}\footnote{\url{https://pandas.pydata.org/}} & \texttt{3.0.3} & Manipolazione ed elaborazione di strutture dati tabulari \\
\texttt{numpy}\footnote{\url{https://numpy.org/}} & \texttt{2.5.1} & Calcolo scientifico e operazioni vettoriali ad alte prestazioni \\
\texttt{scikit-learn}\footnote{\url{https://scikit-learn.org/}} & \texttt{1.9.0} & Pre-elaborazione \textit{feature} e addestramento classificatori \\
\texttt{scipy}\footnote{\url{https://scipy.org/}} & \texttt{1.18.0} & Verifiche di significatività statistica (es. test $t$ di Welch) \\
\texttt{shap}\footnote{\url{https://github.com/slundberg/shap}} & \texttt{0.52.0} & Calcolo dei valori Shapley per l'analisi di \textit{explainability} (XAI) \\
\texttt{joblib}\footnote{\url{https://joblib.readthedocs.io/}} & \texttt{1.5.3} & Serializzazione e caricamento efficiente dei modelli su disco \\
\texttt{matplotlib}\footnote{\url{https://matplotlib.org/}} & \texttt{3.11.1} & Generazione di grafici e diagrammi prestazionali \\
\texttt{seaborn}\footnote{\url{https://seaborn.pydata.org/}} & \texttt{0.13.2} & Visualizzazione avanzata e mappe di calore comparative \\
\hline
\end{tabular}
\end{table}
