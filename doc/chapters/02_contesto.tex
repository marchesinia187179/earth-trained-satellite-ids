    ...
    L'impostazione di un'efficace strategia difensiva richiede pertanto una disamina puntuale dei vincoli strutturali e dei profili di rischio che caratterizzano i diversi contesti trasmissivi, tracciando una distinzione rigorosa tra le peculiarità delle reti di superficie e le criticità emergenti dei canali orbitali.
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[
                ...
                segmentbox/.style={
                    draw=gray!60, dashed, thick, rounded corners=5pt
                },
                ...
                % --- SEGMENTO TERRESTRE ---
                \draw[segmentbox] (-5.6, -2.7) rectangle (5.6, 2.3);
                \node[above right, font=\small\bfseries] at (-5.4, 1.9) {Segmento Terrestre};
                
                % Nodi Terrestri - Riga Inferiore
                \node [nodebox] (core) at (0, 0) {\textbf{Rete Core Terrestre}};
                \node [nidsbox, left=1.5cm of core] (nids_ground) {\textbf{NIDS}\\\textbf{(Segmento Terrestre)}};
                \node [threatbox, right=1.5cm of core] (logic_attack) {\textbf{Attacchi Logici}\\\textbf{(Minaccia)}};
                
                % Nodi Terrestri - Riga Superiore
                ...
                % --- SEGMENTO SPAZIALE ---
                \draw[segmentbox] (-5, 4) rectangle (5, 7);
                \node[above right, font=\small\bfseries] at (-5.5, 6.2) {Segmento Spaziale};

                % Nodi Spaziali
                \node [threatbox, above=3cm of nids_space] (rf_attack) {\textbf{Attacchi RF}};
                \node [nodebox, below left=1cm and 1.5cm of rf_attack] (leo) {\textbf{Costellazione LEO}};
                \node [nodebox, below right=1cm and 1.5cm of rf_attack] (geo) {\textbf{Costellazione GEO}};

                % --- CONNESSIONI SPAZIALI ---
                \draw [dline, ->] (rf_attack) -- (leo);
                \draw [dline, ->] (rf_attack) -- (geo);
                \draw [dline, <->] (leo) -- node[above, font=\footnotesize] {ISL} (geo);

                % --- CANALI RF (Spazio-Terra) ---
                \draw [dline, ->] (leo.south) -- node[right, font=\footnotesize] {Canale RF} (gateway.north);
                \draw [dline, ->] (geo.south) -- node[right, font=\footnotesize] {Canale RF} (terminal.north);

                % --- CONNESSIONI TERRESTRI ---
                \draw [dline, <->] (gateway) -- (nids_space);
                \draw [dline, <->] (terminal) -- (nids_space);

                \draw [line] (gateway.south) -- ++(0,-0.4) -| (core.north);
                \draw [line] (terminal.south) -- ++(0,-0.4) -| (core.north);
