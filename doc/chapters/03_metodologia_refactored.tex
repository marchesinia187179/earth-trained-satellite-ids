...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[]
        ...
        \node [right=0.6cm of d_boot2] (dots_data) {\dots};
        ...
        % Modello Iniziale
        \node [node_primary, below=0.5cm of boosting_title] (f0) {
            $F_0(\mathbf{x})$
        };

        % Stimatori base in cascata
        \node [node_process, below=0.7cm of f0] (tree_b1) {
            $f_1(\mathbf{x})$ addestrato su $r_{i,1}$
        };
        \node [node_process, below=0.7cm of tree_b1] (tree_b2) {
            $f_2(\mathbf{x})$ addestrato su $r_{i,2}$
        };
        \node [below=0.4cm of tree_b2] (dots_boost) {
            \vdots\rule[-0.227cm]{0cm}{0cm}
        };
        \node [node_process, below=0.4cm of dots_boost] (tree_bB) {
            $f_B(\mathbf{x})$ addestrato su $r_{i,B}$
        };

        % Aggregazione finale
        \node [node_output, below=0.7cm of tree_bB] (agg_boosting) {
            Somma: $F_B = F_0 + \nu \sum f_b(\mathbf{x})$
        };
        ...
        \draw [dashed, neutralgrey!70] (0,0.5) -- (0,8);


        \begin{scope}[on background layer]
            % Box Fase 1
            \node[group_box, fit=(d_boot1) (d_boot2) (dots_data) (d_bootB) (tree1) (tree2) (treeB) (agg_bagging)] (g1) {};
        \end{scope}
