...
Nei benchmark di tipo aggregato (sia sorgenti che ibridi), la composizione interna del sottoinsieme malevolo non adotta l'equiprobabilità artificiale tra le classi d'attacco. Al contrario, si preserva la distribuzione naturale a priori $P(A = a \mid Y = 1)$ caratteristica dei domini di origine, evitando alterazioni sintetiche e mantenendo la fedeltà alle frequenze di occorrenza reali delle minacce.
...
\begin{table}[H]
    ...
    Baseline nativa $P_S(\mathbf{x} \mid Y=0, A \in \mathcal{A}_S)$ &
    Minacce sorgente in aggregato $P_S(\mathbf{x} \mid Y=1, A \in \mathcal{A}_S)$ \\ \textbf{+} iniezione altamente sparsa da $\mathcal{D}_T$ ($A \in \mathcal{A}_T$). \\
    \bottomrule
    \end{tabularx}
\end{table}
...
L'intera tassonomia condivide una comune formulazione basata sulla stima della probabilità a posteriori, subordinando l'assegnazione della classe a una soglia decisionale parametrica $\tau$, garantendo così una base di confronto omogenea e indipendente dallo specifico algoritmo di apprendimento sottostante. Per agevolare la consultazione, i formalismi architetturali e i parametri funzionali introdotti in questa sezione sono riepilogati nella Tabella~\ref{tab:notazione_modelli}.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=0.9, every node/.style={transform shape}]
        ...
        \draw[thick, black, smooth, domain=0:10] plot (\x, {4.0*exp(-(\x-2.5)^2/4)}) node[right] {\small $Y=0$};
        % Distribuzione Classe 1 (Malevolo) - Linea tratteggiata
        \draw[thick, black, dashed, smooth, domain=0:10] plot (\x, {3.4*exp(-(\x-6.5)^2/3)}) node[right] {\small $Y=1$};

        % Soglia operativa
        \draw[red, thick] (5.2,0) -- (5.2,4.8);
        \node[red, above] at (5.2,4.8) {\small $\tau$};

        % Aree di confusione
        \fill[blue!10] (5.2,0) -- plot[domain=5.2:10] (\x, {4.0*exp(-(\x-2.5)^2/4)}) -- (10,0) -- cycle;
        \node[black, right] at (9.6,0.4) {\tiny FP};
        \fill[green!10] (0,0) -- plot[domain=0:5.2] (\x, {3.4*exp(-(\x-6.5)^2/3)}) -- (5.2,0) -- cycle;
        \node[black, left] at (1.0,0.8) {\tiny FN};
        \fill[green!10] (0,0) -- plot[domain=0:5.2] (\x, {4.0*exp(-(\x-2.5)^2/4)}) -- (5.2,0) -- cycle;
        \node[black, below] at (2.3,0.2) {\tiny TN};
        \fill[blue!10] (5.2,0) -- plot[domain=5.2:10] (\x, {3.4*exp(-(\x-6.5)^2/3)}) -- (10,0) -- cycle;
        \node[black, right] at (6.8,1.8) {\tiny TP};
    \end{tikzpicture}
    \caption{Esempio illustrativo delle distribuzioni dei punteggi predittivi per le classi nominale e malevola e del ruolo della soglia operativa $\tau$ nella determinazione delle aree di vero positivo, falso positivo, vero negativo e falso negativo.}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        % Asse delle probabilità
        \draw[->, thick] (0,0) -- (7,0) node[right] {\footnotesize $f(\mathbf{x})$};
        \draw[->, thick] (0,0) -- (0,5) node[above] {\footnotesize Frequenza};

        % Distribuzione fittizia della probabilità a posteriori
        \draw[thick, black, smooth, domain=0:6] plot (\x, {2.5*exp(-0.5*(\x-2)^2) + 0.5*exp(-0.6*(\x-4.2)^2)}) node[right] {\small $P(f(\mathbf{x}) \mid Y=1)$};
        \draw[thick, dashed, gray!80, smooth, domain=0:6] plot (\x, {1.8*exp(-0.55*(\x-1.2)^2)}) node[above] {\small $P(f(\mathbf{x}) \mid Y=0)$};

        % Soglia decisionale
        \draw[very thick, red] (3.2,0) -- (3.2,5);
        \node[above, red] at (3.2,5) {\small $\tau$};

        % Aree classi
        \fill[blue!10] (0,0) -- plot[domain=0:3.2] (\x, {1.8*exp(-0.55*(\x-1.2)^2)}) -- (3.2,0) -- cycle;
        \fill[green!10] (3.2,0) -- plot[domain=3.2:6] (\x, {2.5*exp(-0.5*(\x-2)^2) + 0.5*exp(-0.6*(\x-4.2)^2)}) -- (6,0) -- cycle;

        % Legend
        \node[black, align=left] at (5.5,4.5) {\footnotesize $\hat{y}(\tau)=0$ \quad \footnotesize $\hat{y}(\tau)=1$};
    \end{tikzpicture}
