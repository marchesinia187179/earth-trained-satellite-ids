...
L'efficacia dei modelli di Machine Learning applicati ai NIDS in contesti di rete eterogenei poggia fondamentalmente sull'ipotesi che i dati di addestramento e quelli operativi siano indipendenti e identicamente distribuiti (\textit{i.i.d.}). 
Tuttavia, tale condizione viene raramente soddisfatta nei contesti reali, dove la natura dinamica e variabile del traffico determina un marcato scostamento distributivo \cite{pan2010}. 
In tale scenario, come formalizzato nel lavoro di Pan e Yang, l'adozione dei paradigmi di \textit{transfer learning} diventa essenziale per mitigare il decadimento prestazionale dei classificatori, consentendo di trasferire la conoscenza appresa nel dominio di origine senza sostenere l'onere computazionale e logistico legato alla raccolta di nuovi dati etichettati e al riaddestramento integrale dei modelli. 
...
Nelle infrastrutture di telecomunicazione, tale discrepanza teorica si traduce in un'effettiva variazione dei profili di traffico: la transizione dal canale terrestre a quello satellitare introduce elevati tempi di latenza di propagazione, fluttuazioni della larghezza di banda ed effetti stocastici di attenuazione atmosferica. 
Tali dinamiche fisiche alterano significativamente la distribuzione dei tempi di interarrivo dei pacchetti e la struttura dei flussi trasmessi, determinando una modifica diretta nella rappresentazione vettoriale delle \textit{feature} estratte dai NIDS \cite{kodheli2021, azar2023}. 
...
Nelle applicazioni reali di Network Intrusion Detection, la scarsità di campioni appartenenti al dominio di destinazione costituisce un ostacolo primario all'addestramento di modelli nativi, configurando scenari di apprendimento semi-supervisionato o a pochi campioni (\textit{few-shot learning}) \cite{pan2010}. 
