    ...
    \end{table}
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[
                ...
                data_block/.style={rectangle, draw=black!90, fill=white, text width=4cm, text centered, minimum height=1cm, dashed, rounded corners=2pt},
                proc_block/.style={rectangle, draw=black, fill=black!15, text width=3.5cm, text centered, minimum height=1.2cm, rounded corners=5pt},
                seed_node/.style={rectangle, fill=black!70, text=white, rounded corners=3pt, inner sep=4pt, font=\footnotesize\bfseries},
                arrow/.style={->, thick, black!70}
            ]

            % Livello 1: Dati Iniziali
            \node[data_block] (raw_data) at (0, 0) {Traffico Raw (Sorgente \& Target)};
            
            % Nodo Seed deterministico
            \node[seed_node] (seed0) at (0, -1.2) {Seed Fissetto: $s_0$};
            
            % Livello 2: Split e Benchmark Fissi
            \node[proc_block, text width=5cm] (benchmarks) at (0, -2.5) {Partizionamento invariante e Generazione dei Benchmark $\mathcal{B}$};
            
            \draw[arrow] (raw_data) -- (seed0);
            \draw[arrow] (seed0) -- (benchmarks);

            % Livello 3: Nodi Multi-seed e Addestramenti Paralleli
            \node[seed_node] (seed1) at (-4, -4.5) {Seed Stocastico: $s_1$};
            \node[seed_node] (seed2) at (0, -4.5) {Seed Stocastico: $s_2$};
            \node at (2, -4.5) {\dots};
            \node[seed_node] (seedK) at (4, -4.5) {Seed Stocastico: $s_K$};

            \node[proc_block, fill=black!5, text width=2.5cm] (train1) at (-4, -6) {Addestramento Modello $M$};
            \node[proc_block, fill=black!5, text width=2.5cm] (train2) at (0, -6) {Addestramento Modello $M$};
            \node[proc_block, fill=black!5, text width=2.5cm] (trainK) at (4, -6) {Addestramento Modello $M$};

            % Frecce dal blocco Benchmark ai percorsi paralleli
            \draw[arrow, rounded corners] (benchmarks.south) |- (-4, -3.5) -- (seed1);
            \draw[arrow, rounded corners] (benchmarks.south) |- (0, -3.5) -- (seed2);
            \draw[arrow, rounded corners] (benchmarks.south) |- (4, -3.5) -- (seedK);
            ...
            \node[data_block, text width=2.5cm] (eval1) at (-4, -7.5) {Matrice $H_1$};
            \node[data_block, text width=2.5cm] (eval2) at (0, -7.5) {Matrice $H_2$};
            \node[data_block, text width=2.5cm] (evalK) at (4, -7.5) {Matrice $H_K$};
            ...
            \node[proc_block, fill=black!25, text width=5cm] (agg) at (0, -9.5) {Calcolo Stime Campionarie\\ $\hat{\mu}(M, D)$ e $\hat{\sigma}^2(M, D)$};
            
            \draw[arrow, rounded corners] (eval1.south) |- (0, -8.7) -- (agg.north);
            \draw[arrow] (eval2) -- (agg);
            \draw[arrow, rounded corners] (evalK.south) |- (0, -8.7) -- (agg.north);

            % Livello 6: Test
            \node[data_block, draw=black!90, fill=black!10, text width=4cm, minimum height=1cm, dashed=false] (welch) at (0, -11) {Test $t$ di Welch ($p$-value)};
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[
                ...
                attack/.style={draw=black!90, fill=black!30, ellipse, minimum width=1.5cm, minimum height=1cm, inner sep=0},
                shifted/.style={draw=black!70, fill=black!5, ellipse, minimum width=1.5cm, minimum height=1cm, inner sep=0, dashed},
                bnd_agg/.style={draw=black, ultra thick, rounded corners=20pt},
                bnd_bic/.style={draw=black!80, thick, rounded corners=10pt, dotted},
            ...
            \begin{scope}[shift={(0,0)}]
                \node[anchor=south] at (1.5, 3.5) {\textbf{(a) Modello Aggregato ($\Gamma > 1$)}};
                
                % Assi spazio delle caratteristiche X
                \draw[axis] (-1,-2.5) -- (4.5,-2.5) node[right] {$x_1$};
                \draw[axis] (-1,-2.5) -- (-1,3) node[above] {$x_2$};
                
                % Regione decisionale ampia R_1(M_S)
                \draw[bnd_agg, fill=black!10] (-0.5, -2) rectangle (4, 2.5);
                \node[anchor=south west, text=black!90] at (-0.5, 2.5) {$R_1(\mathcal{M}_S)$};

                % Manifold delle distribuzioni
                \node[attack] (a1) at (0.5, 1) {$a_1$};
                \node[attack] (a2) at (1, -1) {$a_2$};
                \node[attack] (a3) at (2.5, 0.5) {$a_3$};

                % Perturbazione (Domain Shift) di a1
                \node[shifted] (a1_shift) at (2, 1.8) {$\tilde{a}_1$};
                \draw[->, thick, black] (a1) -- (a1_shift) node[midway, above left] {$\boldsymbol{\eta}$};
                
                % Didascalia concettuale
                \node[text width=4.5cm, align=center, text=black!80, font=\footnotesize] at (1.5, -3.2) {
            ...
            \begin{scope}[shift={(7.5,0)}]
                \node[anchor=south] at (1.5, 3.5) {\textbf{(b) Modello Biclasse ($\Gamma = 1$)}};
                ...
                \draw[bnd_bic, fill=black!10] (-0.2, 0.2) rectangle (1.2, 1.8);
                \node[anchor=south west, text=black!90] at (-0.2, 1.8) {$R_1(\mathcal{M}_{S,a_1})$};

                % Manifold della distribuzione
                \node[attack] (b1) at (0.5, 1) {$a_1$};

                % Perturbazione (Domain Shift) di a1
                \node[shifted] (b1_shift) at (2, 1.8) {$\tilde{a}_1$};
