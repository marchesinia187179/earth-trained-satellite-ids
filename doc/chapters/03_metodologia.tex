...
\begin{itemize}
    ...
    \item \textbf{un insieme di $\bf{K}$ seed indipendenti $\bf{\mathcal{S} = \{s_1, s_2, \dots, s_K\}}$}, impiegati esclusivamente nelle fasi di addestramento dei classificatori (inizializzazione degli stimatori, campionamento bootstrap, selezione casuale delle caratteristiche), con $K$ inteso come parametro simbolico del framework teorico.
\end{itemize}
...
L'impianto metodologico fin qui descritto culmina nella definizione della matrice $H_k$, che concretizza l'operazione di proiezione incrociata di \textit{tutte} le architetture formulate (siano esse aggregati multiclasse o specialisti biclasse) sull'intera estensione dei domini di test disponibili. Come illustrato nella Tabella~\ref{tab:matrice_incrociata}, lo spazio delle valutazioni si articola in una griglia strutturata che elimina ogni barriera valutativa tra le categorie di classificazione e quelle di benchmark, rendendo esplicita l'universalità dell'insieme di test $\mathcal{B}$.

\begin{table}[H]
    \centering
    \small
    \caption{Rappresentazione concettuale della matrice di valutazione incrociata $H_k \in \mathbb{R}^{|\mathcal{M}| \times |\mathcal{B}|}$.}
    \label{tab:matrice_incrociata}
    \begin{tabular}{l c c c}
        \toprule
        \multirow{2}{*}{$\bf{H_k \in \mathbb{R}^{|\mathcal{M}| \times |\mathcal{B}|}}$} & $\bf{\mathcal{D}^{(\text{\bfseries test})}}$ & $\bf{\mathcal{D}_a^{(\text{\bfseries test})}}$ & $\bf{\mathcal{D}_{b\neq a}^{(\text{\bfseries test})}}$ \\
        & \textbf{(Aggregati)} & \textbf{(Biclasse In-Domain)} & \textbf{(Biclasse Cross)} \\
        \midrule
...
\begin{figure}[H]
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \draw[arrow_flow] (0, 0) -- (6, 0) node[right] {$x_1$};
        \draw[arrow_flow] (0, 0) -- (0, 6) node[above] {$x_2$};
        
        % Regione decisionale ampia R_1(M_S)
        \draw[bnd_agg] (0.5, 0.5) rectangle (5.5, 5.5);
        ...
        \node[node_alert, circle] (a2) at (2.25, 1.75) {$a_2$};
        \node[node_alert, circle] (a3) at (3.75, 3.25) {$a_3$};
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X }
        \toprule
        \textbf{Strumento}  & \textbf{Natura del Requisito} & \textbf{Impatto Tecnico e Motivazione} \\[-5pt]
        & & \textbf{Metodologica} \\
        \midrule
...
\begin{table}[H]
    ...
    \begin{tabular}{c|c c}
        \multicolumn{1}{c|}{%
            \makecell[c]{%
                \textbf{Classe Predetta ($\hat{y}(\tau)$)}\\[-5pt]
                $\times$\\[-5pt]
                \textbf{Classe Reale ($y$)}
            }
        }
        & \textbf{Positivo ($y=1$)}
        & \textbf{Negativo ($y=0$)} \\
        \hline
        \parbox[c][\confusionheight][c]{\confusionwidth}{%
            \centering
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[global_font]
        ...
        \begin{scope}[shift={(0,0)}]
            % Assi
            \draw[arrow_flow] (0,0) -- (5.5,0) node[right, text=neutralgrey] {\footnotesize $FPR$};
            \draw[arrow_flow] (0,0) -- (0,5.5) node[above, text=neutralgrey] {\footnotesize $TPR$};

            % Griglia di riferimento
            \draw[neutralgrey!30, thin] (0,4.5) -- (4.5,4.5) -- (4.5,0);
            \node[below left, text=neutralgrey] at (0,0) {\footnotesize $0$};
            \node[below, text=neutralgrey] at (4.5,0) {\footnotesize $1$};
            \node[left, text=neutralgrey] at (0,4.5) {\footnotesize $1$};

            % Area sottesa (AUC)
            \fill[lightblue] (0,0) -- plot[smooth, domain=0:4.5] (\x, {4.5*sqrt(\x/4.5)}) -- (4.5,0) -- cycle;

            % Baseline casuale (diagonale)
            \draw[dashed, accentorange, thick] (0,0) -- (4.5,4.5);

            % Curva ROC
            \draw[very thick, mainblue, smooth, domain=0:4.5] plot (\x, {4.5*sqrt(\x/4.5)});

            % Etichette
            \node[text=mainblue, font=\small\bfseries] at (2.3, 1.2) {$AUC_{ROC}$};
            \node[above, text=neutralgrey] at (2.75, 5.5) {\textbf{(a) Curva ROC}};
        \end{scope}

        % --- Sottofigura (b): Curva Precision-Recall ---
        \begin{scope}[shift={(7.5,0)}]
            % Assi
            \draw[arrow_flow] (0,0) -- (5.5,0) node[right, text=neutralgrey] {\footnotesize $Recall\ (TPR)$};
            \draw[arrow_flow] (0,0) -- (0,5.5) node[above, text=neutralgrey] {\footnotesize $Precision$};
            ...
            % Area sottesa (AUC)
            \fill[lightblue] (0,0) -- plot[smooth, domain=0:4.5] (\x, {4.5 - 2*(\x/4.5)^2}) -- (4.5,0) -- cycle;
            
            % Baseline prevalenza (linea orizzontale)
            \draw[dashed, accentorange, thick] (0,0.8) -- (4.5,0.8) node[right, text=accentorange] {\footnotesize $\pi$};

            % Curva PR
            \draw[very thick, mainblue, smooth, domain=0:4.5] plot (\x, {4.5 - 2*(\x/4.5)^2});

            % Etichette
            \node[text=mainblue, font=\small\bfseries] at (1.2, 1.2) {$AUC_{PR}$};
            \node[above, text=neutralgrey] at (2.75, 5.5) {\textbf{(b) Curva PR}};
        \end{scope}
    \end{tikzpicture}
...
Nel Capitolo~\ref{ch:implementazione}, il framework teorico viene pertanto istanziato ed eseguito empiricamente. 
