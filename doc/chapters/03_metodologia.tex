...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3cm} >{\raggedright\arraybackslash}X }
        ...
        Operatore di proiezione e combinazione dalle \textit{feature} grezze allo spazio $\mathcal{X}$ \\
        $N(\cdot)$ &
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l >{\raggedright\arraybackslash}X}
        ...
        $\tau \in (0, 1)$ &
        Soglia decisionale parametrica per il bilanciamento dei falsi positivi \\
        $\mathcal{M}_{\text{base}}$ &
        Modello baseline, addestrato sul \textit{Set di Iniezione} $\mathcal{D}_{\text{inj}}^{(\text{train})}$ \\
        $\mathcal{M}_{S}$ &
        Modello aggregato nativo sorgente, addestrato sul \textit{Set Sorgente} $\mathcal{D}_{S}^{(\text{train})}$ \\
        $\mathcal{M}_{H}^{(T)}$ &
        ...
        Modelli biclasse (sorgente e ibridi), addestrati sulla componente nominale con una singola famiglia d'attacco $a \in \mathcal{A}$ \\
        $\mathcal{L}$ &
