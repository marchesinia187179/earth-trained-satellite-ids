...
\label{ch:metodologia}
    ...
    Il presente capitolo definisce il framework analitico, diagnostico e statistico adottato per caratterizzare lo scostamento distributivo (\textit{domain shift}), decodificare le logiche decisionali dei modelli e valutare la significatività delle variazioni prestazionali tra domini eterogenei[cite: 3]. L'impianto metodologico integra tecniche di riduzione dimensionale, stima non parametrica di densità e attribuzione dell'importanza delle caratteristiche fondata sulla teoria dei giochi cooperativi, affiancandole a un formalismo rigoroso per l'operatività a soglia e la verifica di ipotesi statistiche[cite: 3].
    Per agevolare la consultazione della trattazione, i simboli matematici, i costrutti diagnostici e le metriche prestazionali introdotti in questa sezione sono riepilogati nella Tabella~\ref{tab:notazione_framework}[cite: 3].
    ...
    \begin{table}[H]
        ...
        \midrule
        $\mathbf{Z}_D$ & 
        Matrice dei dati del benchmark $D$ standardizzata mediante la media $\boldsymbol{\mu}_S$ e la deviazione standard $\boldsymbol{\sigma}_S$ del solo dominio sorgente[cite: 3] \\
        
        $\boldsymbol{\Sigma}_D, \mathbf{W}_D$ & 
        Matrice di covarianza campionaria e matrice di proiezione ortogonale sui primi due assi della PCA[cite: 3] \\
        
        $\hat{f}_y(z), h_{\text{KDE}}$ & 
        Stima di densità univariata KDE per la classe $y$ e relativo parametro di lisciamento (\textit{bandwidth}) con kernel gaussiano $\kappa(\cdot)$[cite: 3] \\
        
        $\phi_j(\mathbf{x})$ & 
        Valore SHAP associato alla $j$-esima caratteristica del vettore $\mathbf{x}$, rappresentante il suo contributo marginale esatto[cite: 3] \\
        
        $I_j^{(D)}, C_j$ & 
        Importanza media assoluta SHAP della $j$-esima caratteristica sul benchmark $D$ e frequenza globale di inclusione nella \textit{top-3}[cite: 3] \\
        
        $TP(\tau), FP(\tau), TN(\tau), FN(\tau)$ & 
        Cardinalità della matrice di confusione determinate in funzione della soglia decisionale parametrica $\tau$[cite: 3] \\
        
        $AUC_{ROC}, AUC_{PR}$ & 
        Area sottesa alle curve \textit{Receiver Operating Characteristic} e \textit{Precision-Recall} per la caratterizzazione al variare di $\tau \in [0,1]$[cite: 3] \\
        
        $\Delta F_1$ & 
        Variazione prestazionale in termini di $F_1$-score per la quantificazione del degrado da \textit{covariate shift} tra sorgente $\mathcal{D}_S$ e target $\mathcal{D}_T$[cite: 3] \\
        
        $t, \nu, H_0$ & 
        Statistica del test $t$ di Welch, gradi di libertà effettivi di Welch--Satterthwaite e ipotesi nulla di equivalenza delle medie prestazionali[cite: 3] \\
        \bottomrule
        \end{tabularx}
    \end{table}
        ...
        Pertanto, preliminarmente all'estrazione delle proiezioni e alla stima densometrica, viene applicata una normalizzazione dedicata a ciascun benchmark $D \in \mathcal{B}$, secondo l'insieme dei benchmark di test già introdotto nella Sezione~\ref{subsubsec:metriche_aggregazione}. Tale trasformazione è ancorata unicamente alle statistiche del dominio sorgente $\mathcal{D}_S$ e viene mantenuta rigorosamente disaccoppiata dalla pipeline di addestramento dei modelli.
        ...
        L'ancoraggio a $(\boldsymbol{\mu}_S, \boldsymbol{\sigma}_S)$, anziché a statistiche ricalcolate localmente su ciascun benchmark $D$, garantisce che eventuali traslazioni di media o contrazioni di varianza presenti nel benchmark $D$ non vengano annullate artificialmente: una standardizzazione locale ricentrerebbe infatti ogni benchmark sulla propria origine, occultando esattamente lo scostamento distributivo che l'analisi si propone di rendere visibile. Si osserva che tale ancoraggio è una scelta di riferimento per la sola finalità diagnostico-visiva, e non implica alcuna dipendenza dalla scala da parte del classificatore stesso: come dimostrato nella Sezione~\ref{subsubsec:invarianza_monotonica}, la decisione dei modelli ad albero è invariante rispetto a qualsiasi trasformazione monotonica crescente, ancorata o meno al dominio sorgente. L'ancoraggio a $(\boldsymbol{\mu}_S, \boldsymbol{\sigma}_S)$ risponde dunque esclusivamente all'esigenza di disporre di un sistema di riferimento fisso e interpretabile per l'ispezione visiva, non a un requisito del modello di classificazione.
        ...
        Sia $\mathcal{T}_D \subset \mathcal{F}$ l'insieme contenente le prime tre caratteristiche classificate nel benchmark $D$. Per identificare i descrittori maggiormente resilienti e discriminanti ai confini tra domini $\mathcal{D}_S$ e $\mathcal{D}_T$, è stato implementato un contatore di frequenza globale $C_j$:
        \begin{equation}
        ...
        Per valutare analiticamente le prestazioni dei modelli di classificazione formalizzati nella sezione precedente e quantificare l'impatto del \textit{domain shift}, si definisce un framework di valutazione fondato sulla teoria della decisione statistica. In particolare, si distingue la valutazione puntuale delle prestazioni, riferita a una specifica soglia operativa, dall'analisi del comportamento discriminativo del classificatore al variare della soglia di decisione.

        \subsubsection{Formalizzazione della Matrice di Confusione}
        \label{subsubsec:decisione_parametrica}

        Richiamando la regola di decisione parametrica introdotta nella Sezione~\ref{subsubsec:formulazione_erm}, la classe predetta $\hat{y}_i(\tau)$ consente di definire le cardinalità della matrice di confusione, espresse come funzioni della soglia operativa $\tau$:
        ...
        \begin{table}[H]
            \centering
            \caption{Struttura della Matrice di Confusione parametrizzata rispetto alla soglia operativa $\tau$.}
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[scale=0.9, every node/.style={transform shape}]
                % Assi cartesiani
                \draw[->, thick] (0,0) -- (8.5,0) node[right] {\small $f(\mathbf{x}) \in [0,1]$};
                \draw[->, thick] (0,0) -- (0,3.5) node[above] {\small $p(f(\mathbf{x}) \mid y)$};

                % Distribuzione Classe 0 (Lecito) - Linea continua
                \draw[thick, black, smooth, domain=0.2:5.5] plot (\x, {3.0*exp(-(\x-2.0)^2/0.8)});
                \node at (1.8, 2.5) [anchor=south] {\small $y=0$ (Traffico Lecito)};

                % Distribuzione Classe 1 (Malevolo) - Linea tratteggiata
                \draw[thick, dashed, black!80, smooth, domain=2.5:8.0] plot (\x, {2.5*exp(-(\x-5.5)^2/1.2)});
                \node at (6.0, 2.0) [anchor=south] {\small $y=1$ (Traffico Malevolo)};

                % Soglia decisionale tau
                \draw[very thick, gray!80!black] (3.8, 0) -- (3.8, 3.2);
                \node[above] at (3.8, 3.2) {\small Soglia $\tau$};
                \fill (3.8,0) circle (2pt) node[below] {$\tau$};

                % Regioni decisionali
                \draw[<->, thick, shorten >=2pt, shorten <=2pt] (0, -0.6) -- (3.75, -0.6) node[midway, below] {\footnotesize Decisione: $\hat{y}(\tau)=0$};
                \draw[<->, thick, shorten >=2pt, shorten <=2pt] (3.85, -0.6) -- (8.2, -0.6) node[midway, below] {\footnotesize Decisione: $\hat{y}(\tau)=1$};
            \end{tikzpicture}
        ...
        La valutazione puntuale delle metriche presuppone la scelta di uno specifico valore della soglia operativa (ad esempio $\tau=0.5$). Una caratterizzazione più completa del comportamento discriminativo del classificatore richiede invece l'analisi dell'intero intervallo di valori della soglia, $\tau\in[0,1]$.
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                \begin{scope}[shift={(0,0)}]
                    % Assi
                    \draw[->, thick] (0,0) -- (3.8,0) node[right] {\footnotesize $FPR$};
                    \draw[->, thick] (0,0) -- (0,3.8) node[above] {\footnotesize $TPR$};
                    
                    % Griglia di riferimento
                    \draw[gray!30, thin] (0,3.2) -- (3.2,3.2) -- (3.2,0);
                    \node[below left] at (0,0) {\tiny $0$};
                    \node[below] at (3.2,0) {\tiny $1$};
                    \node[left] at (0,3.2) {\tiny $1$};
                    
                    % Area sottesa (AUC) in toni di grigio
                    \fill[gray!20] (0,0) -- plot[smooth, domain=0:3.2] (\x, {3.2*sqrt(\x/3.2)}) -- (3.2,0) -- cycle;
                    
                    % Baseline casuale (diagonale)
                    \draw[dashed, gray!80] (0,0) -- (3.2,3.2);
                    
                    % Curva ROC
                    \draw[thick, black, smooth, domain=0:3.2] plot (\x, {3.2*sqrt(\x/3.2)});
                    
                    % Etichette
                    \node at (1.8, 1.0) {\footnotesize $AUC_{ROC}$};
                    \node[above] at (1.6, 3.4) {\textbf{(a) Curva ROC}};
                \end{scope}

                % --- Sottofigura (b): Curva Precision-Recall ---
                \begin{scope}[shift={(5.2,0)}]
                    % Assi
                    \draw[->, thick] (0,0) -- (3.8,0) node[right] {\footnotesize $Recall\ (TPR)$};
                    \draw[->, thick] (0,0) -- (0,3.8) node[above] {\footnotesize $Precision$};
                    ...
                    % Area sottesa (AUC) in toni di grigio
                    \fill[gray!20] (0,3.2) -- plot[smooth, domain=0:3.2] (\x, {3.2 - 2.2*(\x/3.2)^2}) -- (3.2,0) -- (0,0) -- cycle;
                    
                    % Baseline prevalenza (linea orizzontale)
                    \draw[dashed, gray!80] (0,0.8) -- (3.2,0.8) node[right] {\tiny $\pi$};
                    
                    % Curva PR
                    \draw[thick, black, smooth, domain=0:3.2] plot (\x, {3.2 - 2.2*(\x/3.2)^2});
                    
                    % Etichette
                    \node at (1.2, 1.2) {\footnotesize $AUC_{PR}$};
                    \node[above] at (1.6, 3.4) {\textbf{(textbf{b}) Curva PR}};
                \end{scope}
            \end{tikzpicture}
