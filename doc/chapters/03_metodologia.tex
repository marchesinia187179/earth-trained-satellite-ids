...
\label{ch:metodologia}
    ...
    \label{sec:framework_analitico}
        ...
        \begin{equation}
            \mathbf{Z}_{k, ij} = \frac{\mathbf{X}_{k, ij} - \mu_{S, j}}{\sigma_{S, j} + \epsilon}
        ...
        \begin{enumerate}
            ...
            \item \textbf{Calibrazione stabile della Bandwidth ($h$):} La condivisione dello spazio adimensionale ($z \sim \mathcal{N}(0,1)$ sul dominio sorgente) stabilizza l'ottimizzazione della larghezza di banda del kernel $h$, prevenendo fenomeni di \textit{undersmoothing} (una banda eccessivamente stretta, che produce una curva frastagliata e sovra-adattata al singolo campione osservato) o di \textit{oversmoothing} selettivo (una banda eccessivamente ampia, che appiattisce dettagli distributivi rilevanti) tra variabili a diversa varianza grezza.
        \end{enumerate}
        ...
        \end{equation}
        dove $h$ è il parametro di lisciamento (\textit{bandwidth}) e $K(\cdot)$ è la funzione kernel, per la quale si adotta la forma gaussiana standard:
        \begin{equation}
