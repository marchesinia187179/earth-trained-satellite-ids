...
\end{table}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            partte/.style={
                rectangle, draw=black!75, fill=gray!28, semithick, 
                align=center, font=\footnotesize, inner sep=4pt
            },
            synth/.style={
                rectangle, draw=black!90, fill=gray!12, thick, 
                align=center, rounded corners=2pt, inner sep=6pt
            },
            ...
            \node [domnode] (source) at (-4, 11) {
                \textbf{Dominio Sorgente $\mathcal{D}_S$}\\
                $P_S(\mathbf{x}\mid Y{=}0, A\in\mathcal{A}_S) \cup P_S(\mathbf{x}\mid Y{=}1, A\in\mathcal{A}_S)$
            };
            \node [domnode] (target) at (4, 11) {
                \textbf{Domini Target $\mathcal{D}_T$}\\
                $P_T(\mathbf{x}\mid Y{=}1, A\in\mathcal{A}_T)$
            };

            % RIGA 2: Isolamento e Split Logico
            \node [process] (split_s) at (-4, 9) {
                \textbf{Isolamento Logico (Runtime)}\\
                Split Stratificato Train/Test
            };
            \node [process] (split_t) at (4, 9) {
                \textbf{Isolamento Logico (Runtime)}\\
                Split Stratificato Train/Test
            };

            % RIGA 3: Partizioni Disgiunte
            \node [parttr] (ds_tr) at (-5, 7) {$\mathcal{D}_S^{(train)}$};
            \node [partte] (ds_te) at (-3, 7) {$\mathcal{D}_S^{(test)}$};
            \node [parttr] (dt_tr) at ( 3, 7) {$\mathcal{D}_T^{(train)}$};
            \node [partte] (dt_te) at ( 5, 7) {$\mathcal{D}_T^{(test)}$};

            % RIGA 4: Sintesi Parallela dei Benchmark (In-Memory)
            \node [synth] (synth_tr) at (-4, 4) {
                \textbf{Sintesi Logica (Train)}\\
                Composizione da: $\mathcal{D}_S^{(train)}$ e $\mathcal{D}_T^{(train)}$\\
                \textit{Asimmetria volumetrica: Ratio $\rho$}\\
                \textit{Prior naturali in aggregato}
            };

            \node [synth] (synth_te) at (4, 4) {
                \textbf{Sintesi Logica (Test)}\\
                Composizione da: $\mathcal{D}_S^{(test)}$ e $\mathcal{D}_T^{(test)}$\\
                \textit{Asimmetria volumetrica: Ratio $\rho$}\\
                \textit{Prior naturali in aggregato}
            };

            % RIGA 5: Categorie Risultanti
            \node [rectangle, draw=black!90, fill=gray!25, thick, rounded corners=2pt, align=center, inner sep=5pt] (categories) at (0, 0) {
                \textbf{5 Tipologie di Benchmark Istanziati a Runtime (Train / Test disgiunti):}\\
                1. Nativi Sorgente Aggregati \quad$\vert$\quad 2. Nativi Sorgente Biclasse\\
                3. Ibridi Aggregati \quad$\vert$\quad 4. Ibridi Biclasse\\
                5. Iniezione
            };
            ...
            % Connessioni ai Blocchi di Sintesi
            \draw [sublinearr] (ds_tr.south) -- ++(0,-0.3) -| ($(synth_tr.north)+(-0.8,0)$);
            \draw [sublinearr] (dt_tr.south) -- ++(0,-0.3) -| ($(synth_tr.north)+( 0.8,0)$);

            \draw [sublinearr] (ds_te.south) -- ++(0,-0.3) -| ($(synth_te.north)+(-0.8,0)$);
            \draw [line] (dt_te.south) -- ($(synth_te.north)+(1,0)$);
            
            % Collegamento alle categorie finali
            \draw [sublinearr] (synth_tr.south) |- (categories.west);
            \draw [sublinearr] (synth_te.south) |- (categories.east);

        \end{tikzpicture}
        }
        \caption{Pipeline logica astratta: isolamento preventivo delle partizioni per la prevenzione del \textit{data leakage}, vincoli di proporzionamento parametrico $\rho$ e sintesi in-memory delle cinque tipologie di benchmark.}
