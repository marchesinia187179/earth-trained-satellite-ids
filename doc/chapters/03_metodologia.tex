...
\label{sec:tassonomia_modelli}
    ...
    \begin{enumerate}
        ...
        \end{equation}
        in cui la classe nominale $Y=0$ di origine terrestre è unita alla totalità dei vettori d'attacco $Y=1$ del contesto satellitare di destinazione ($\mathcal{D}_{T}^{(\text{train})}$). Il rispettivo vettore di parametri ottimali risponde alla formulazione:
        \begin{equation}
    ...
    Si precisa che i benchmark biclasse (categorie 2 e 4 della tassonomia, cfr. Sezione~\ref{subsec:composizione_domini}) non rientrano nella famiglia di classificatori aggregati qui trattata; la loro formalizzazione, in qualità di addestramento specialistico e/o partizioni di valutazione per famiglia d'attacco, è oggetto della Sezione~\ref{subsec:modelli_biclasse}.
