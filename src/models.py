...
def _get_standardized_model_name(dataset_type, unique_classes, model_type):
    ...
    :param model_type: string indicating the algorithm type
    ...
    if len(classes) > 2:
        return f"{model_type}_model_{dataset_type}_aggregate"
    ...
    return f"{model_type}_model_{dataset_type}_{attack_name}"


def _save_metadata(model, model_name, metrics, dataset_type, classes, samples):
    """
    Saves the metadata of the trained model, including its parameters and evaluation metrics, to a CSV file.

    :param model: the trained model object (RandomForestClassifier or IsolationForest)
    ...
    results = {
        ...
        # Specific parameters: Random Forest
...
def _isolation_forest(data):
    """
    Trains an Isolation Forest model on the provided dataset and returns the trained model, 
    evaluation metrics, and feature names.
    ...
    # Select labels for evaluation
    y_test = test_set[MLConstants.Y_LABEL]

    # Build and fit the model (Unsupervised: we fit only on X_train)
    model = IsolationForest(
        random_state=MLConstants.RANDOM_STATE,
        verbose=MLConstants.MODEL_VERBOSE,
        n_jobs=-1  # Parallelizza il calcolo per velocizzare l'esecuzione
    )
    model.fit(X_train)

    # Get raw predictions (1: normal, -1: anomaly)
    raw_pred = model.predict(X_test)
    
    # Map predictions to project standard (1 -> 0 for Normal, -1 -> 1 for Anomaly)
    y_pred = np.where(raw_pred == -1, 1, 0)

    # Calculate anomaly scores where higher values mean "more anomalous" (for ROC-AUC/metrics)
...
def model_processing(data, dataset_type, model_type):
    ...
    if model_type == Naming.RANDOM_FOREST:
        model, metrics, feature_names = _random_forest(data)
    elif model_type == Naming.ISOLATION_FOREST:
        model, metrics, feature_names = _isolation_forest(data)
