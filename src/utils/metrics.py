...
from .config import MLConstants, Naming
...
def calculate_welch_ttest_from_summary(data, alpha=MLConstants.WELCH_TTEST_ALFA_VALUE):
    """
    ...
    """
    # Get from DataFrame the first model_name with the word Naming.INJECTION (ignoring lowercase and uppercase letters)
    matching_models = data[data["model_name"].str.contains(Naming.INJECTION, case=False, na=False)]["model_name"].unique()

    if len(matching_models) == 0:
        print(f"Error: No reference model containing 'injection' found in the DataFrame.")
        return []
    
    # Select the first matching model found as a reference.
