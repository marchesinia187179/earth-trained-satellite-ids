...
def _stratified_split(data):
    ...
    for _, group in data.groupby('class'):
        ...
        group = group.sample(frac=1, random_state=MLConstants.MAIN_SEED).reset_index(drop=True)
...
def data_preprocessing(data, dataset_type):
    ...
    data = data.sample(frac=1, random_state=MLConstants.MAIN_SEED).reset_index(drop=True)
